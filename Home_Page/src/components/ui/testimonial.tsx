...
export function TestimonialCarousel({
  ...
  return (
    <section
      ...
      className={cn('relative w-full overflow-hidden rounded-2xl p-6', className)}
    >
      {/* arrows */}
      {showArrows && count > 1 && (
        <>
          <button
            ...
            className='absolute left-3 top-1/2 -translate-y-1/2 rounded-full p-2 transition focus-visible:outline-none focus-visible:ring-2'
          >
          ...
          <button
            ...
            className='absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-2 transition focus-visible:outline-none focus-visible:ring-2'
          >
          ...
          {items.map((_, i) => {
            ...
            return (
              <button
                ...
                onClick={() => setIndex(i)}
                className={cn(
                  'h-2.5 w-2.5 rounded-full ring-1 transition focus-visible:outline-none focus-visible:ring-2',
                  is ? 'opacity-100' : 'opacity-60 hover:opacity-80',
                )}
              />
            );
          })}
...
function DefaultCard({
  ...
  const accent = item.accent ?? '#C2410C';
  ...
  return (
    ...
    >
        ...
        <div className='relative mb-3'>
          {item.avatar ? (
            <img
              ...
              className={cn('h-16 w-16 rounded-full object-cover', avatarClassName)}
              style={ring}
              loading='lazy'
            />
          ) : (
            <div
              className={cn('grid h-16 w-16 place-items-center rounded-full', avatarClassName)}
              style={ring}
            >
      ...
      {item.name && (
        <h3 className={cn('text-xl font-semibold tracking-tight', nameClassName)}>
          {item.name}
        </h3>
      )}
      {item.role && (
      ...
      {item.quote && (
        <p className={cn('text-balance max-w-2xl text-sm leading-7', quoteClassName)}>
          {item.quote}
        </p>
      )}
    </article>
